...
(function () {
    ...
    StepNavigatorUI.prototype.setStepNodeSizes = function (steps) {
        ...
        //console.log(document.body.offsetWidth+"---"+document.querySelector(this.containerID).offsetWidth);
    ...
    StepNavigatorUI.prototype.updatestepActivity = function (stepno, status) {
        ...
        if (status == STATUS_INACTIVE) {
            ...
            stepnode.classList.remove('activateStep', 'delay');
            progressbar.classList.add('inactivateStep', 'delay');
            progressbar.classList.remove('activateStep');
            currentObj.inProgress = false;
        } else if (status == STATUS_INPROGRESS) {
            progressbar.classList.remove('inactivateStep', 'delay');
            progressbar.classList.add('inprogressStep');
            stepnode.classList.remove('inactivateStep');
            stepnode.classList.add('inprogressStep', 'delay');
            currentObj.inProgress = true;
        } else if (status == STATUS_ACTIVE) {
            progressbar.classList.remove('inactivateStep', 'inprogressStep', 'delay');
            progressbar.classList.add('activateStep');
            stepnode.classList.remove('inactivateStep', 'inprogressStep');
            stepnode.classList.add('activateStep', 'delay');
            currentObj.inProgress = false;
        } else if (status == STATUS_RESET) {
            stepnode.classList.remove('inactivateStep', 'inprogressStep', 'activateStep');
            progressbar.classList.remove('inactivateStep', 'inprogressStep', 'activateStep');
            currentObj.inProgress = false;
        }
    };
    ...
    StepNavigatorUI.prototype.configureCSS = function () {
        var stepNavigatorCSS = "body{\
                            ...
                            .delay{\
                                transition-delay: "+this.animationDuration+";\
                            }\
                            .inactivateStep{\
                                color: #000;\
